...
################ PLOT FOR ALL KELCH ################
df_pred <- df_predict_cutoff_allyear %>% 
  filter(year >= min_year +  pred_y, Locus == "K13")  
ex <- pfk7_data %>% 
  filter(is.finite(lrsmed), nobs > numofobs, District %in% unique(df_pred$District), Locus == "K13")
ex_pt <- pfk7_data %>% 
  filter(nobs > numofobs, District %in% unique(df_pred$District), Locus == "K13") %>% #, year >= min_year & year <= min_year+4
  mutate(point_col = case_when(year >= min_year & year <= min_year + 2 ~ "#E69F00",
                               year > min_year + 2 & year <= min_year + 3 ~ "#56B4E9",
                               year > min_year + 3 & year <= min_year + 4 ~ "#0072B2",
                               year > min_year + 4 ~ "black",
...
prev_cutoff_c580y_Pailin_plot <- 
  ...
  geom_line(data=df_Pailin_forecasting, aes(year, exp(predict)*100/(1+exp(predict)), group = label, color = label)) +
  geom_ribbon(data = df_Pailin_forecasting, aes(ymin = conf.low_prev, ymax = conf.high_prev, x = year, color = label, group = label, fill = label), linetype = "dashed", alpha = 0.2) +
  geom_point(data = ex_pt, aes(x=year, y=prev*100, size = n), color = ex_pt$point_col) +
  labs(shape="Mutations", size = "Sample Size") +
  scale_x_continuous(breaks = seq(min(ex_pt$year), max(df_Pailin_forecasting$year), by = 2)) +
  theme_bw() +
  ylab("Prevalence") +
  xlab("Year") +
  ggpubr::theme_pubclean(base_size = 12) +
