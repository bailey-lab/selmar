...
fig6_prev_cutoff_plot <- 
  ggplot() +
  geom_point(data = ex_pt, aes(x=year, y=prev*100, color = Locus, group = Locus)) +
  scale_size_binned(name = "Sample Size", range = c(0.2,2)) +
  geom_line(data = df_true, aes(year, predict_prev, color = Locus, group = Locus)) + 
  geom_line(data = df_pred, aes(year, predict_prev, color = Locus, group = Locus)) +
  geom_ribbon(data = df_pred, aes(ymin = conf.low_prev, ymax = conf.high_prev, x = year, color = Locus, group = Locus, fill = Locus), linetype = "dashed", alpha = 0.2) +
  theme_bw() +
  facet_wrap(~District) +
  ylab("Prevalence") +
  xlab("Year") +
