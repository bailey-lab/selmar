...
cam<-x %>% dplyr::filter(country %in% c("Cambodia","Myanmar","Laos","Vietnam","Thailand")) %>%
...
cam$c580y[which(cam$ARTresistant=="null" | cam$ARTresistant=="undetermined")]<-NA

cam$r539t<-0
cam$r539t[grep("539t|539T",cam$kelch13_349.726_ns_changes)]<-1
cam$r539t[which(cam$kelch13_349.726_ns_changes=="-")]<-NA
cam$r539t[which(cam$ARTresistant=="null" | cam$ARTresistant=="undetermined")]<-NA

## all validated markers.
cam$all_kelch<-0
cam$all_kelch[which(cam$kelch13_349.726_ns_changes=="-")]<-NA
cam$all_kelch[grep("580Y|580y|446|458|476|493|539|543|553|561",cam$kelch13_349.726_ns_changes)]<-1
cam$all_kelch[which(cam$ARTresistant=="null" | cam$ARTresistant=="undetermined")]<-NA
...
pfk7_data <-cam %>%
  ...
  dplyr::mutate(prev_580y=c580y/N,
                prev_539t=r539t/N,
...
pfk7_data <- pfk7_data %>% rename(District = site) %>% rename(n = N) %>%
...
write.table(pfk7_data,"analysis/data/pfk7_data.txt")
