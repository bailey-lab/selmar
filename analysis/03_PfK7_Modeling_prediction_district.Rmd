...
## Figure 4 (B): Preah Vihear - Avg Forecasting 
### Preprocessing
```{r}
df_pred <- df_predict_cutoff_allyear %>% 
...
# TEST - Manuscript figures 
## Figure 4 (A): Pailin - Model
### Preprocessing
```{r}
prev_Pailin <- pfk7_data %>% 
  select(country, District, year, Locus, x, n, N, nobs, lrsmed, prev, predict_stan_prev) %>% 
  filter(District == "Pailin", Locus == "C580Y", nobs > numofobs)
```
...
df_Pailin_pred <- df_predict_cutoff_allyear %>% 
  ...
  mutate(predict_prev = exp(predict)*100/(1+exp(predict))) 
ex_pt <- pfk7_data %>% 
  filter(nobs > numofobs, District == "Pailin", Locus == "C580Y") %>%
  mutate(point_col = case_when(year >= min_year & year <= min_year + 2 ~ "#E69F00",
                               year > min_year + 2 & year <= min_year + 3 ~ "#56B4E9",
                               year > min_year + 3 & year <= min_year + 4 ~ "#0072B2",
                               year > min_year + 4 ~ "black",
...
```{r}
ggsave(paste0(
    "figures/Figure4-ForecastingOverview/Figure4A_C580Y_Pailin_",
    prev_cutoff_perc,
    "prev_model.png"
  ),
  prev_Pailin_plot,
...
## Figure 4 (B): Pailin - Avg Forecasting 
### Preprocessing
```{r}
df_pred <- df_predict_cutoff_allyear %>% 
  filter(year >= min_year + pred_y, Locus == "C580Y", District == "Pailin", Locus == "C580Y") %>%
  mutate(predicted_prev = exp(predict)*100/(1+exp(predict)))
ex <- pfk7_data %>% 
  filter(is.finite(lrsmed), nobs > numofobs, District %in% unique(df_pred$District), Locus == "C580Y")
ex_pt <- pfk7_data %>% 
...
ggsave(
  paste0(
    "figures/Figure4-ForecastingOverview/Figure4B_C580Y_Pailin_",
    prev_cutoff_perc,
    "prev_forecasting.png"
  ),
  prev_cutoff_c580y_Pailin_plot,
