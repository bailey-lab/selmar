...
for (i in pred_year) {
  ...
  new_data <- pfk7_data %>%
    ...
    ungroup() %>% 
    select(adj_year, year, District, country, min_year_c580y, Locus) %>%
  ...
  new_data <- pfk7_data %>%
    ...
    ungroup() %>% 
    select(adj_year, year, District, country, min_year_r539t, Locus) %>%
  ...
  new_data <- pfk7_data %>%
    ...
    ungroup() %>% 
    select(adj_year, year, District, country, min_year_all_kelch, Locus) %>%
...
write.table(df_predict_allyear, paste0(
    "figures/predictions/SE_Asia/per_district/20_cutoff_firstobs/SEAsia_pred_C580Y_",
    prev_cutoff_perc,
    "predict_allyears.txt"))
```
...
for (i in pred_year) {
  ...
  selected_district <- val$District[val$prev_580y <= prev_cutoff]
  ...
  # forecasting
  new_data <- pfk7_data %>%
    filter(nobs_c580y > numofobs, is.finite(pfk7_data$lrsmed_580y), District %in% selected_district) %>%
    mutate(Locus = "C580Y") %>%
    group_by(District, Locus) %>%
    complete(year = seq(min(min_year_c580y) + i, 2023, 1)) %>% # this creates all the year values up to 2023
    ungroup() %>% 
    select(adj_year, year, District, min_year_c580y, Locus) %>%
    mutate(min_year_c580y = replace(min_year_c580y, is.na(min_year_c580y), 0)) %>%
    group_by(District) %>%
    distinct(year, .keep_all = TRUE)
  ...
  selected_district <- val$District[val$prev_539t <= prev_cutoff]
  ...
  # forecasting
  new_data <- pfk7_data %>%
    filter(nobs_r539t > numofobs, is.finite(pfk7_data$lrsmed_539t), District %in% selected_district) %>% 
    mutate(Locus = "R539T") %>%
    group_by(District, Locus) %>%
    complete(year = seq(min(min_year_r539t) + i, 2023, 1)) %>% # this creates all the year values up to 2023
    ungroup() %>% 
    select(adj_year, year, District, min_year_r539t, Locus) %>%
    mutate(min_year_r539t = replace(min_year_r539t, is.na(min_year_r539t), 0)) %>%
    group_by(District) %>%
    distinct(year, .keep_all = TRUE)
  ...
  selected_district <- val$District[val$prev_all_kelch <= prev_cutoff]
  ...
  # forecasting
  new_data <- pfk7_data %>%
    filter(nobs_kelch > numofobs, is.finite(pfk7_data$lrsmed_all_kelch), District %in% selected_district) %>% 
    mutate(Locus = "all_kelch") %>%
    group_by(District, Locus) %>%
    complete(year = seq(min(min_year_all_kelch) + i, 2023, 1)) %>% # this creates all the year values up to 2023
    ungroup() %>% 
    select(adj_year, year, District, min_year_all_kelch, Locus) %>% 
    mutate(min_year_all_kelch = replace(min_year_all_kelch, is.na(min_year_all_kelch), 0)) %>%
    group_by(District) %>%
    distinct(year, .keep_all = TRUE)
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_cutoff_firstobs/SEAsia_pred_R539T_",
    prev_cutoff_perc,
    "prev_pred.png"
  ),
  prev_r539t_plot,
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_cutoff_firstobs/SEAsia_pred_PfK13_",
    prev_cutoff_perc,
    "prev_pred.png"
  ),
  prev_all_kelch_plot,
...
## Based on the first 2, 3, and 4 years with prevalence cut off averaged per District
```{r}
pred_year <- c(2, 3, 4)
prev_cutoff <- 0.2
...
for (i in pred_year) {
  ...
  new_data <- pfk7_data %>%
    ...
    complete(year = seq(min(min_year_c580y) + i, 2023, 1)) %>% # this creates all the year values up to 2023
    ungroup() %>% 
    select(adj_year, year, District, min_year_c580y, Locus) %>% # select just the columns needed for the model
    mutate(min_year_c580y = replace(min_year_c580y, is.na(min_year_c580y), 0)) %>%
    group_by(District) %>%
    distinct(year, .keep_all = TRUE)
  ...
  new_data <- pfk7_data %>%
    ...
    complete(year = seq(min(min_year_r539t) + i, 2023, 1)) %>% # this creates all the year values up to 2023
    ungroup() %>% 
    select(adj_year, year, District, min_year_r539t, Locus) %>% # select just the columns needed for the model
    mutate(min_year_r539t = replace(min_year_r539t, is.na(min_year_r539t), 0)) %>%
    group_by(District) %>%
    distinct(year, .keep_all = TRUE)
  ...
  new_data <- pfk7_data %>%
    ...
    complete(year = seq(min(min_year_all_kelch) + i, 2023, 1)) %>% # this creates all the year values up to 2023
    ungroup() %>% 
    select(adj_year, year, District, min_year_all_kelch, Locus) %>% # select just the columns needed for the model
    mutate(min_year_all_kelch = replace(min_year_all_kelch, is.na(min_year_all_kelch), 0)) %>%
    group_by(District) %>%
    distinct(year, .keep_all = TRUE)
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_avg/SEAsia_pred_C580Y_",
    prev_cutoff_perc,
    "avg_prev_pred.png"
  ),
  prev_c580y_plot,
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_avg/SEAsia_pred_R539T_",
    prev_cutoff_perc,
    "avg_prev_pred.png"
  ),
  prev_r539t_plot,
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_avg/SEAsia_pred_PfK13_",
    prev_cutoff_perc,
    "avg_prev_pred.png"
  ),
  prev_all_kelch_plot,
...
# Comparison to predictions without restrictions
## Pf7 Data preprocessing
```{r}
corr_df_cutoff <- NULL
for(l in unique(df_predict_cutoff_allyear$label)) {
  true <- df_predict_allyear %>% rename(t_pred = predict)
  pred <- df_predict_cutoff_allyear %>%
    filter(label == l) %>%
    arrange(label, Locus, District, year) %>%
  ...
  df <- cbind(true, pred$p_pred)
  colnames(df)[ncol(df)] <- "p_pred"
  corr_df_cutoff <- rbind(corr_df_cutoff, df)
}

corr_df_cutoff <- corr_df_cutoff %>% mutate(p_pred_prev = exp(p_pred) * 100 / (1 + exp(p_pred)),
                                            t_pred_prev = exp(t_pred) * 100 / (1 + exp(t_pred)))
...
corr_df_avg <- corr_df_avg %>% mutate(p_pred_prev = exp(p_pred) * 100 / (1 + exp(p_pred)),
                                      t_pred_prev = exp(t_pred) * 100 / (1 + exp(t_pred)))
```
...
correlation_pred_plot_cutoff <- ggplot(corr_df_cutoff, aes(x = t_pred_prev, y = p_pred_prev, color = label, group = label)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  stat_cor(aes(color = label), label.x = 20, size = 2) +
  facet_wrap(~ District + Locus) +
  xlab("true") +
  ylab("predicted") +
...
correlation_pred_plot_avg <- ggplot(corr_df_avg, aes(x = t_pred_prev, y = p_pred_prev, color = label, group = label)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  stat_cor(aes(color = label), label.x = 30, , size = 2) +
  facet_wrap(~ District + Locus) +
  xlab("true") +
  ylab("predicted") +
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_cutoff_firstobs/correlation/prev/SEAsia_correlation_pred_",
    prev_cutoff_perc,
    "cutoff_prev.png"
  ),
  correlation_pred_plot_cutoff,
  width = 8,
  height = 5,
  dpi = 700
)
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_avg/correlation/prev/SEAsia_correlation_pred_",
    prev_cutoff_perc,
    "avg_prev.png"
  ),
  correlation_pred_plot_avg,
  width = 8,
  height = 5,
  dpi = 700
)
...
```{r}
## Correlation plot for 20% cutoff in first year
correlation_raw_plot_cutoff <- ggplot(corr_df_cutoff, aes(x = t_pred_prev, y = p_pred_prev, color = label, group = label)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  stat_cor(aes(color = label), label.x = 20, size = 2) +
  facet_wrap(~ District + Locus) +
...
## Correlation plot for 20% avg
correlation_raw_plot_avg <- ggplot(corr_df_avg, aes(x = t_pred_prev, y = p_pred_prev, color = label, group = label)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  stat_cor(aes(color = label), label.x = 30, , size = 2) +
  facet_wrap(~ District + Locus) +
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_cutoff_firstobs/correlation/raw/SEAsia_correlation_raw_",
    prev_cutoff_perc,
    "cutoff_prev.png"
  ),
  correlation_raw_plot_cutoff,
  width = 8,
  height = 5,
  dpi = 700
)
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/20_avg/correlation/raw/SEAsia_correlation_raw_",
    prev_cutoff_perc,
    "avg_prev.png"
  ),
  correlation_raw_plot_avg,
  width = 8,
  height = 5,
  dpi = 700
)
