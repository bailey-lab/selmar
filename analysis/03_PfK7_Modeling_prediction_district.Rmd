...
ex_pt <- pfk7_data %>% 
  filter(nobs > numofobs, District %in% unique(df_pred$District), Locus == "R539T") %>%
  mutate(point_col = case_when(year >= min_year & year <= min_year + 2 ~ "#E69F00",
                               year > min_year + 2 & year <= min_year + 3 ~ "#56B4E9",
                               year > min_year + 3 & year <= min_year + 4 ~ "#0072B2",
                               year > min_year + 4 ~ "black",
...
ex_pt <- pfk7_data %>% 
  filter(nobs > numofobs, District %in% unique(df_pred$District), Locus == "PfK13") %>%
  mutate(point_col = case_when(year >= min_year & year <= min_year + 2 ~ "#E69F00",
                               year > min_year + 2 & year <= min_year + 3 ~ "#56B4E9",
                               year > min_year + 3 & year <= min_year + 4 ~ "#0072B2",
                               year > min_year + 4 ~ "black",
...
for (mu in unique(corr_df_cutoff$Locus)){
  correlation_raw_plot_cutoff <- corr_df_cutoff %>% filter(Locus == mu) %>% 
    ggplot(aes(x = t_pred_prev, y = p_pred_prev, color = label, group = label, linetype = "dashed")) +
    geom_point() +
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/50_cutoff_firstobs/correlation/SEAsia_cutoff_raw_large_C580Y_correlation_raw_",
    prev_cutoff_perc,
    "cutoff_prev.pdf"
  ),
  C580Y_correlation_raw_plot_cutoff,
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/50_cutoff_firstobs/correlation/SEAsia_cutoff_raw_large_R539T_correlation_raw_",
    prev_cutoff_perc,
    "cutoff_prev.pdf"
  ),
  R539T_correlation_raw_plot_cutoff,
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/50_cutoff_firstobs/correlation/SEAsia_cutoff_raw_large_PfK13_correlation_raw_",
    prev_cutoff_perc,
    "cutoff_prev.pdf"
  ),
  PfK13_correlation_raw_plot_cutoff,
...
ex_pt <- pfk7_data %>% 
  filter(nobs > numofobs, District == "Binh Phuoc") %>%
  mutate(point_col = case_when(year >= min_year & year <= min_year + 2 ~ "#E69F00",
                               year > min_year + 2 & year <= min_year + 3 ~ "#56B4E9",
                               year > min_year + 3 & year <= min_year + 4 ~ "#0072B2",
                               year > min_year + 4 ~ "black",
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/50_cutoff/SEAsia_cutoff_pred_C580Y_BinhPhuoc_",
    prev_cutoff_perc,
    "avg_prev_pred.png"
  ),
  prev_cutoff_c580y_BinhPhuoc_plot,
