...
prev_c580y_plot <- 
  ggplot() +
  geom_line(data=ex %>% mutate(N = n+x), aes(year, predict_stan_prev, color = "true"), linewidth = 1) + 
...
prev_cutoff_c580y_plot <- 
  ...
  geom_line(data=df_pred, aes(year, exp(predict)*100/(1+exp(predict)), group = label,color = label), linetype = "dashed") +
  geom_point(data = ex_pt %>% mutate(N = n+x), aes(x=year, y=prev*100, size = N), color = ex_pt$point_col) +
  labs(shape="Mutations", size = "Sample Size") +
  theme_bw() +
  facet_wrap(~District) +
  ylab("Prevalence") +
...
prev_cutoff_c580y_plot <- 
  ...
  geom_line(data=ex, aes(year, predict_stan_prev, color = "true"), linewidth = 1) + 
  geom_line(data=df_pred, aes(year, exp(predict)*100/(1+exp(predict)), group = label,color = label), linetype = "dashed") +
  geom_point(data = ex_pt %>% mutate(N = n+x), aes(x=year, y=prev*100), color = ex_pt$point_col) +
  labs(shape="Mutations", size = "Sample Size") +
  theme_bw() +
  facet_wrap(~District) +
  ylab("Prevalence") +
...
for (mu in unique(corr_df_cutoff$Locus)){
  correlation_raw_plot_avg <- corr_df_cutoff %>% 
    filter(Locus == mu) %>% 
...
prev_BinhPuoc <- pfk7_data %>% 
  select(country, District, year, Locus, x, n, nobs, lrsmed, prev, predict_stan_prev, conf.low_prev, conf.high_prev) %>% 
  filter(District == "Binh Phuoc", Locus == "C580Y")
...
### Ploting
```{r}
df_BinhPhuoc_pred <- df_predict_cutoff_allyear %>% 
  filter(year <= min_year + pred_y, Locus == "C580Y", District == "Binh Phuoc") 
ex_pt <- pfk7_data %>% 
  filter(nobs > numofobs, District == "Binh Phuoc", Locus == "C580Y") %>%
  mutate(point_col = case_when(year >= min_year & year <= min_year + 2 ~ "#E69F00",
                               year > min_year + 2 & year <= min_year + 3 ~ "#56B4E9",
                               year > min_year + 3 & year <= min_year + 4 ~ "#0072B2",
                               year > min_year + 4 ~ "black",
...
prev_BinhPuoc_plot <- ggplot() +
  geom_line(data = df_BinhPhuoc_pred, aes(year, exp(predict)*100/(1+exp(predict)), group = label, color = label)) +
...
```{r}
df_pred <- df_predict_cutoff_allyear %>% 
  filter(year >= min_year + pred_y, Locus == "C580Y", District == "Binh Phuoc", Locus == "C580Y") %>%
  mutate(predicted_prev = exp(predict)*100/(1+exp(predict)))
ex <- pfk7_data %>% 
  filter(is.finite(lrsmed), nobs > numofobs, District %in% unique(df_pred$District), Locus == "C580Y")
ex_pt <- pfk7_data %>% 
  filter(nobs > numofobs, District %in% unique(df_pred$District), Locus == "C580Y") %>%
  mutate(point_col = case_when(year >= min_year & year <= min_year + 2 ~ "#E69F00",
                               year > min_year + 2 & year <= min_year + 3 ~ "#56B4E9",
                               year > min_year + 3 & year <= min_year + 4 ~ "#0072B2",
                               year > min_year + 4 ~ "black",
...
### Plotting
```{r}
prev_cutoff_c580y_BinhPhuoc_plot <- 
  ggplot() +
  geom_line(data=ex %>% mutate(N = n+x), aes(year, predict_stan_prev, color = "true"), linewidth = 1) + 
  geom_line(data=df_pred, aes(year, exp(predict)*100/(1+exp(predict)), group = label, color = label), linetype = "dashed") +
  geom_point(data = ex_pt, aes(x=year, y=prev*100, size = N), color = ex_pt$point_col) +
...
ggsave(
  paste0(
    "figures/predictions/SE_Asia/per_district/50_cutoff_BinhPhuoc/SEAsia_cutoff_pred_C580Y_BinhPhuoc_",
    prev_cutoff_perc,
    "avg_prev_pred.png"
  ),
  prev_cutoff_c580y_BinhPhuoc_plot,
