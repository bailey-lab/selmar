...
prev_plot <- prev_df  %>% mutate(N = n+x) %>%
  ...
  scale_y_log10() +
  scale_color_manual(values=c("#56B4E9", "#E69F00", "#009E73"), labels = c("580Y", "combined PfK13", "539T")) + 
...
ggsave(paste0(fileLocManuscript_pfK7, "/Pfk7_prevalence.jpeg"), prev_plot, width = 8, height = 6, dpi=700)
```
...
effect_size <- rbind(effect_size, stan_eff %>% mutate(mutation = "all_kelch"))
...
coef_all_kelch <- ranef(all_kelch_stanmod)$District %>% mutate(District = rownames(ranef(all_kelch_stanmod)$District), mutation = "all_kelch", estimate = ranef(all_kelch_stanmod)$District$adj_year, conf.low = stan_eff_all_kelch$conf.low, conf.high = stan_eff_all_kelch$conf.high) %>% 
  select(conf.low, estimate, conf.high, mutation, District) %>% 
  remove_rownames()
...
selec_coef$estimate[selec_coef$mutation == "all_kelch"] <- selec_coef$estimate[selec_coef$mutation == "all_kelch"] + effect_size_bay$estimate[effect_size_bay$mutation == "all_kelch"]
...
selec_coef$conf.low[selec_coef$mutation == "all_kelch"] <- selec_coef$conf.low[selec_coef$mutation == "all_kelch"] + effect_size_bay$conf.low[effect_size_bay$mutation == "all_kelch"]
...
selec_coef$conf.high[selec_coef$mutation == "all_kelch"] <- selec_coef$conf.high[selec_coef$mutation == "all_kelch"] + effect_size_bay$conf.high[effect_size_bay$mutation == "all_kelch"]
...
effect_size_bay <- effect_size_bay %>% relocate(District, mutation)

ggsave(paste0(fileLocManuscript_pfK7, "/PfK7_Baysian_Effectsize.jpeg"), width = 8.7, height = 5, dpi=700)
write.table(effect_size_bay, paste0(fileLocManuscript_pfK7, "/PfK7_Baysian_SelectionCoefficients.txt"))

gt_bay <- gt(effect_size_bay[,1:5])
gt_bay <- gt_bay %>% tab_header(
    title = md("**Selection Coefficient based on Baysian GLM**")
  )
gt_bay
gtsave(gt_bay, paste0(fileLocManuscript_pfK7, "PfK7_Baysian_SelectionCoef_bay.png"))
```
...
min_allkelch <- min(effect_size_bay %>% filter(mutation == "all_kelch" & District != "Southeast Asia") %>% pull(estimate))
max_allkelch <- max(effect_size_bay %>% filter(mutation == "all_kelch" & District != "Southeast Asia") %>% pull(estimate))
min_allkelch
max_allkelch
