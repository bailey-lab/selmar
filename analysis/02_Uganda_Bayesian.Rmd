...
```{r}
prev <- df %>%
  filter(is.finite(lrsmed) & nobs > numofobs) %>%
...
coef_a675v <- ranef(a675v_stanmod)$District %>% 
  mutate(District = rownames(ranef(a675v_stanmod)$District), 
         mutation = "A675V", 
         estimate = ranef(a675v_stanmod)$District$year, 
...
coef_c469y <- ranef(c469y_stanmod)$District %>% 
  mutate(District = rownames(ranef(c469y_stanmod)$District), 
         mutation = "C469Y", 
         estimate = ranef(c469y_stanmod)$District$year, 
...
coef_c469f <- ranef(c469f_stanmod)$District %>% 
  mutate(District = rownames(ranef(c469f_stanmod)$District), 
         mutation = "C469F",
         estimate = ranef(c469f_stanmod)$District$year, 
...
# effect_size_bay_plot <- effect_size_bay 
# effect_size_bay_plot$conf.high[effect_size_bay_plot$conf.high > 1] <- 1.1
# effect_size_bay_plot$conf.low[effect_size_bay_plot$conf.low < -1] <- -1.1
...
max_469Y <- max(effect_size_bay %>% 
                  filter(mutation == "469Y" & District != "Uganda") %>% 
                  pull(estimate))
selec_ceof_stats[2, ] <- cbind("469Y", min_469Y, effect_size_bay[which(effect_size_bay$estimate == min_469Y),][1], max_469Y, effect_size_bay[which(effect_size_bay$estimate == max_469Y),][1])
