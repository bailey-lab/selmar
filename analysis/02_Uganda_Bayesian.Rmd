...
```{r cars}
#load data
df<- read.table("data/Ugandan_data.txt")
df_comb <- read.table("data/Ugandan_comb_data.txt")
...
#Modeling using stan - Baysian Generalized Linear Model
```{r}
...
prev_Uganda <- df_comb %>%
  filter(is.finite(lrsmed) & nobs > numofobs) %>% mutate(N = n+comb) %>%
  ggplot(aes(year, med*100)) +
  geom_point(aes(size = N)) + 
  labs(shape="Mutations", size = "Sample Size") +
  facet_wrap(~District) +
  theme_bw() +
  geom_line(aes(year, exp(predict_stan_Uganda)*100/(1+exp(predict_stan_Uganda))), size = 1) + 
  geom_ribbon(aes(ymin = exp(conf.low_uganda)*100/(1+exp(conf.low_uganda)), ymax = exp(conf.high_uganda)*100/(1+exp(conf.high_uganda))), alpha = 0.2,  show.legend = FALSE) +
  scale_y_log10() +
  ylab("Prevalence") +
  xlab("Year") +
  ggpubr::theme_pubclean(base_size = 12) +
...
pdf(paste0(fileLocManuscript, "/Figure3_Effectsize.pdf"), width = 8, height = 5)
write.table(effect_size_bay, paste0(fileLocManuscript, "/SelectionCoefficients.txt"))
effect_size_bay <- read.table(paste0(fileLocManuscript, "/SelectionCoefficients.txt"))

gt_bay <- gt(effect_size_bay[,1:5])
gt_bay <- gt_bay %>% tab_header(
    title = md("**Selection Coefficient based on Baysian GLM**")
  )
gtsave(gt_bay, paste0(fileLocManuscript,"SelectionCoef_bay.png"))
```
