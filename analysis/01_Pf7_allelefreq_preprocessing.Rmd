...
merged_kelch <- kelch13 %>% 
  ...
  summarise(n = n(),
            WT = sum(1-k13),
            K13 = sum(k13)/(sum(k13) + WT),
            C580Y = sum(C580Y)/(sum(C580Y) + WT),
            other_K13 = sum(F446I + Y493H + I543T + P553L + R561H + P574L+ A675V)/(sum(F446I + Y493H + I543T + P553L + R561H + P574L + A675V) + WT),
            R539T = sum(R539T)/(sum(R539T) + WT))  %>%
  ungroup %>%
  select(-WT) %>%
  pivot_longer(cols = K13:R539T) %>%
  rename(freq = value,
         Locus = name,
         District = site) %>%
