...
pie_chart_size <- c(8, 8, 6.5, 4.5, 3, 4, 3)
count <- 0
for (y in 2016:2022){
  count <- count + 1
...
map_SEAsia <- ggplot() +
  ...
  geom_scatterpie_legend(pie_data$radius_adj*2,
                         x = 108,
                         y = 24, labeller = function(x) round(100*x/2)) +
...
# Figure 5: Overview of data plot SE Asia (PfK7 data)
```{r}
...
```{r}
ggsave(
  paste0(fileLocManuscript, "/supplement/Figure5-SEAsia-DataOverview-NotAnanlysis/SEAsia_dataoverview.pdf"),
...
ggsave(
  paste0(fileLocManuscript, "/supplement/Figure5-SEAsia-DataOverview-NotAnanlysis/SEAsia_dataoverview.png"),
  plot_overview_seAsia_notincluded,
  width = 8,
  height = 8.5,
  dpi = 700
