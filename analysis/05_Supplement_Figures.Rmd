...
# Figure1: Overview of data plot Uganda with more than two observations 
```{r}
...
# Figure2: Overview of data plot Uganda with less than three observations
```{r}
plot_overview_uganda <- df_uganda %>%
  filter(nobs <= numofobs) %>% 
  ggplot(aes(year, med * 100, group = Locus, color = Locus)) +
...
for (y in 2016:2021){
  ...
  xlab("") +
  ylab("") +
  geom_scatterpie(data = pie_chart_df, aes(x=long, y=lat, group=District, r = radius_adj), cols=c("A675V", "C469F", "C469Y", "R561H")) +
  geom_scatterpie_legend(pie_chart_df$radius*val, x=28.7, y=2.3, labeller = function(x) round(x * 100/val)) +
