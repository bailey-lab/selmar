...
PfK13_pred <- rstanarm::stan_glmer(
  ...
  data = df %>% 
    filter(Locus == "K13" & nobs > numofobs),
...
new_data <- df %>%
  filter(nobs > numofobs, 
         is.finite(df$lrsmed), 
         Locus == "K13") %>% # this is for c469Y model
...
new_data <- df %>%
  filter(nobs > numofobs, 
         is.finite(df$lrsmed), 
         Locus == "K13") %>% # this is for PfK13 model
  group_by(District, Locus) %>% # we group by the District and Locus
  complete(year = seq(max(year), 2033, 1)) %>% # this creates all the year values up to 2023
  select(adj_year, year, District, Locus, min_year, max_year) %>%
  mutate(min_year = replace(min_year, is.na(min_year), 0),
...
pred_PfK13 <- ggplot() +
  geom_line(data = df_true, aes(year, predict_prev, color = "#CC79A7")) +
  geom_line(data = df_pred, aes(year, predict_prev, color = "#CC79A7")) +
  ...
  geom_point(data = ex_pt, aes(x = year, y = med * 100, color = "#CC79A7")) +
  theme_bw() +
  facet_wrap(~District) +
  ylab("K13 Prevalence") +
  xlab("Year") +
...
pred_a675v <- ggplot() +
  geom_line(data = df_true, 
            aes(year, predict_prev, color = "#CC79A7")) +
  ...
  geom_point(data = ex_pt, 
             aes(x = year, y = med * 100, color = "#CC79A7")) +
  theme_bw() +
  facet_wrap(~District) +
