...
## Obtain confidence interval for forecasting
```{r}
#  get model predictions for plotting
df_predict_allyear$conf.low <- NA
df_predict_allyear$conf.high <- NA

c469y_conf <- df_predict_allyear$predict %>% apply(2, quantile, prob = (c(0.025, 0.975)))
c469f_conf <- rstanarm::posterior_predict(c469f_pred, type = "response") %>% apply(2, quantile, prob = (c(0.025, 0.975)))
a675v_conf <- rstanarm::posterior_predict(a675v_pred, type = "response") %>% apply(2, quantile, prob = (c(0.025, 0.975)))

# here just add predictions  for the c469Y rows
df_predict_allyear$conf.low[is.finite(df$lrsmed) & df$Locus == "C469Y" & df$nobs > numofobs] <- c469y_conf[1,]
df_predict_allyear$conf.low[is.finite(df$lrsmed) & df$Locus == "A675V" & df$nobs > numofobs] <- a675v_conf[1,]
df_predict_allyear$conf.low[is.finite(df$lrsmed) & df$Locus == "C469F" & df$nobs > numofobs] <- c469f_conf[1,]

df_predict_allyear$conf.high[is.finite(df$lrsmed) & df$Locus == "C469Y" & df$nobs > numofobs] <- c469y_conf[2,]
df_predict_allyear$conf.high[is.finite(df$lrsmed) & df$Locus == "A675V" & df$nobs > numofobs] <- a675v_conf[2,]
df_predict_allyear$conf.high[is.finite(df$lrsmed) & df$Locus == "C469F" & df$nobs > numofobs] <- c469f_conf[2,]
```
...
pred_a675v <- ggplot() +
  ...
  geom_line(data = df_pred,
    aes(year, exp(predict) * 100 / (1 + exp(predict)), group = label, color = label),
    linetype = "dashed") +
...
pred_c469y <- ggplot() +
  geom_line(data = ex %>% mutate(N = n + x),
    aes(year, predict_stan_prev, color = "true"),
    linewidth = 1) +
  geom_line(data = df_pred, aes(year, exp(predict) * 100 / (1 + exp(predict)), group = label, color = label),
    linetype = "dashed"
  ) +
  geom_point(data = ex_pt, aes(x = year, y = med * 100)) +
  #ggtitle("C469Y") +
...
################ PLOT FOR C469F ################
ex <-df %>% 
  filter(is.finite(lrsmed), nobs > numofobs, Locus == "C469F") 
ex_pt <-df %>% 
  filter(nobs > numofobs, Locus == "C469F") 
df_pred <-df_predict_allyear %>% 
  filter(District %in% unique(ex$District), Locus == "C469F", year >= min_year + pred_y)
