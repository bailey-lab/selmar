...
predictions <- rstanarm::posterior_predict(c469y_stanmod, type = "response", newdata = new_data) %>% colMeans
new_data$predict <- predictions
...
predictions <- rstanarm::posterior_predict(a675v_stanmod, type = "response", newdata = new_data) %>% colMeans
new_data$predict <- predictions
...
predictions <- rstanarm::posterior_predict(c469f_stanmod, type = "response", newdata = new_data) %>% colMeans
new_data$predict <- predictions
...
df_pred <- df_predict %>% filter(District %in% unique(ex$District), Locus == "A675V", year > 2020) 
## PLOT FOR A675V
pred_a675v<- ggplot() +
  ...
  geom_vline(data=ex, aes(xintercept= 2021), linetype = "dashed") +
...
df_pred <- df_predict %>% filter(District %in% unique(ex$District), Locus == "C469Y", year > 2020) 
## PLOT FOR C469Y
pred_c469y <- ggplot() +
  geom_line(data=ex %>% mutate(N = n+x), aes(year, predict_stan_prev, color = "true")) + 
  geom_line(data=df_pred, aes(year, exp(predict)*100/(1+exp(predict)), color = "predicted"), linetype = "dashed") +
...
df_pred <- df_predict %>% filter(District %in% unique(ex$District), Locus == "C469F", year > 2020) 
## PLOT FOR C469F
pred_c469f <- ggplot() +
  geom_line(data=ex %>% mutate(N = n+x), aes(year, predict_stan_prev, color = "true")) + 
  geom_line(data=df_pred, aes(year, exp(predict)*100/(1+exp(predict)), color = "predicted"), linetype = "dashed") +
