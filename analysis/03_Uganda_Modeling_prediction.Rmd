...
df_pred <- df_predict_allyear %>% filter(District %in% unique(ex$District), Locus == "A675V", year >= min_year + pred_y)
## PLOT FOR A675V
pred_a675v <- ggplot() +
...
ex <- df %>% filter(is.finite(lrsmed), nobs > numofobs, Locus == "C469Y") #%>% mutate(pt_color = case_when (Locus == "A675V" ~ "#55C667FF", Locus == "C469F" ~ "#DCE319FF"))
df_pred <- df_predict_allyear %>% filter(District %in% unique(ex$District), Locus == "C469Y", year >= min_year + pred_y)
## PLOT FOR C469Y
pred_c469y <- ggplot() +
  geom_line(data=ex %>% mutate(N = n+x), aes(year, predict_stan_prev, color = "true"), linewidth = 1) + 
  geom_line(data=df_pred, aes(year, exp(predict)*100/(1+exp(predict)), group = label,color = label), linetype = "dashed") +
...
ex <- df %>% filter(is.finite(lrsmed), nobs > numofobs, Locus == "C469F") #%>% mutate(pt_color = case_when (Locus == "A675V" ~ "#55C667FF", Locus == "C469F" ~ "#DCE319FF"))
df_pred <- df_predict_allyear %>% filter(District %in% unique(ex$District), Locus == "C469F", year >= min_year + pred_y)
## PLOT FOR C469F
pred_c469f <- ggplot() +
  geom_line(data=ex %>% mutate(N = n+x), aes(year, predict_stan_prev, color = "true"), linewidth = 1) + 
  geom_line(data=df_pred, aes(year, exp(predict)*100/(1+exp(predict)), group = label,color = label), linetype = "dashed") +
...
ggsave("figures/predictions/Uganda_pred_A675V_pred.jpeg", pred_a675v, width = 8, height = 5, dpi=700)
ggsave("figures/predictions/Uganda_pred_C469Y_pred.jpeg", pred_c469y, width = 8, height = 5, dpi=700)
ggsave("figures/predictions/Uganda_pred_C469F_pred.jpeg", pred_c469f, width = 8, height = 5, dpi=700)

```


#PREDICTION FORECASTING WITH 20% PREVALENCE
```{r}
pred_year <- c(2, 3, 4)
...
## PLOT PREDICTION ON PREVALENCE WITH 20% PREVALENCE
```{r}
# Plot the predictions in each site with only Bayesian
#convert to prevalence scale
df$predict_stan_prev <- exp(df$predict_stan)*100/(1+exp(df$predict_stan))
...
ggsave("figures/predictions/Uganda_pred_A675V_20prev_pred.jpeg", pred_a675v, width = 8, height = 5, dpi=700)
ggsave("figures/predictions/Uganda_pred_C469Y_20prev_pred.jpeg", pred_c469y, width = 8, height = 5, dpi=700)
ggsave("figures/predictions/Uganda_pred_C469F_20prev_pred.jpeg", pred_c469f, width = 8, height = 5, dpi=700)
