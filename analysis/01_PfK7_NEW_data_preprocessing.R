...
df_c580y <- cam %>%
  select(sample_id, year, country, site, c580y, ARTresistant) %>%
...
df_r539t <- cam %>%
  select(sample_id, year, country, site, r539t, ARTresistant) %>%
  arrange(country, site, year) %>%
  group_by(country, site, year) %>%
  summarise(N = n(),
...
cam$all_kelch[grep("580Y|580y|446|458|469|476|493|539|543|553|561|574|622|675", #validated markers determined by WHO
                   cam$kelch13_349.726_ns_changes)] <- 1
df_all_kelch <- cam %>%
  select(sample_id, year, country, site, all_kelch, ARTresistant) %>%
  arrange(country, site, year) %>%
  group_by(country, site, year) %>%
  summarise(N = n(),
...
cam$other_kelch[grep("446|458|469|476|493|543|553|561|574|622|675",
                   cam$kelch13_349.726_ns_changes)] <- 1
df_other_kelch <- cam %>%
  select(sample_id, year, country, site, other_kelch, ARTresistant) %>%
  arrange(country, site, year) %>%
  group_by(country, site, year) %>%
  summarise(N = n(),
