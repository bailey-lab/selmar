...
df <-  df_raw %>% select(-x_org, -value) %>%
  rbind(df_comb %>% mutate(Locus = "K13") %>% rename(x = comb))
...
K13_stanmod <- rstanarm::stan_glmer(
  lrsmed ~ adj_year + (1 + adj_year | District),
  data = df %>% filter(Locus == "K13" & nobs > numofobs),
  weights = wt_med, adapt_delta = 0.9975
)
vals <- summary(K13_stanmod$stanfit)$summary[,1]
vals <- vals[grep("^b", names(vals))]
ranef_K13_stanmod<- data.frame("Intercepts" = vals[seq(1, length(vals), 2)],
           "Slopes" = vals[seq(2, length(vals), 2)])
ranef_K13_stanmod$mutation <- rep("K13", dim(ranef_K13_stanmod)[1])
...
ranef_stanmod <- rbind(ranef_c469y_stanmod, ranef_a675v_stanmod, ranef_c469f_stanmod, ranef_p441l_stanmod, ranef_K13_stanmod)
...
```{r}
prev <- df %>%
  filter(is.finite(lrsmed) & nobs > numofobs & Locus != "K13") %>%
...
ggsave(
  paste0(fileLocManuscript, "/Figure2-UgandaPrevalence/Figure2_Uganda_prevalence_K13.png"),
  prev_K13,
  width = 8,
  height = 5,
  dpi = 700
...
ggsave(
  paste0(fileLocManuscript, "/Figure2-UgandaPrevalence/Figure2_Uganda_prevalence_K13.pdf"),
  prev_K13,
  width = 8,
  height = 5,
  dpi = 700
...
stan_eff_K13 <- as.data.frame(K13_stanmod$stan_summary[grep("year District", rownames(K13_stanmod$stan_summary)),]) %>% select(c("2.5%","97.5%")) %>% setNames(c("conf.low", "conf.high")) %>% slice(1:(n()-1))
...
selec_coef <- rbind(coef_a675v, coef_c469y, coef_c469f, coef_p441l, coef_K13)
...
min_K13 <- min(effect_size_bay %>% 
                  filter(mutation == "K13" & District != "Uganda") %>% 
                  pull(estimate))
max_K13 <- max(effect_size_bay %>% 
                  filter(mutation == "K13" & District != "Uganda") %>% 
                  pull(estimate))
selec_ceof_stats[5, ] <- cbind("K13", min_K13, effect_size_bay[which(effect_size_bay$estimate == min_K13),][1], max_K13, effect_size_bay[which(effect_size_bay$estimate == max_K13),][1])
