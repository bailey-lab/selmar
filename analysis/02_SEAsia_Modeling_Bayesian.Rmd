...
prev_plot <- prev_df  %>% 
  ...
  theme_bw() +
  geom_line(data = prev_df %>% filter(nobs > numofobs, is.finite(lrsmed)) %>% mutate(N = n+x), aes(year, predict_stan_prev, color = Locus, group = Locus), size = 1) +
  geom_ribbon(data = prev_df %>% filter(nobs > numofobs, is.finite(lrsmed)) %>% mutate(N = n+x), aes(ymin = conf.low_prev, ymax = conf.high_prev, color = Locus, fill = Locus), alpha = 0.2,  show.legend = FALSE) +
  scale_color_manual(
...
```{r}
ggsave(
  paste0(fileLocManuscript_pfK7, "/supplement/Figure7-SEAsiaPrevalence/SEAsia_posterior_prevalence.png"),
  prev_plot,
  width = 8,
  height = 8,
  dpi = 700
...
ggsave(
  paste0(fileLocManuscript_pfK7, "/supplement/Figure7-SEAsiaPrevalence/SEAsia_posterior_prevalence.pdf"),
  prev_plot,
  width = 8,
  height = 8,
  dpi = 700
