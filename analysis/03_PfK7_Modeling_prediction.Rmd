...
c580y_pred <- rstanarm::stan_glmer(
  lrsmed_580y ~ adj_year + (1 + adj_year | District),
  data = pfk7_data %>% filter(nobs_c580y > numofobs, year >= min_year_c580y & year <= min_year_c580y+6),
  weights = wt_med_580y, adapt_delta = 0.9975
)
...
new_data <- pfk7_data %>%
  ...
  complete(year = seq(min(min_year_c580y)+6, 2023, 1)) %>% # this creates all the year values up to 2023
...
r539t_pred <- rstanarm::stan_glmer(
  lrsmed_539t ~ adj_year + (1 + adj_year | District),
  data = pfk7_data %>% filter(nobs_r539t > numofobs, year >= min_year_r539t & year <= min_year_r539t+6),
  weights = wt_med_539t, adapt_delta = 0.9975
)
...
new_data <- pfk7_data %>%
  ...
  mutate(Locus = "R539T") %>%
  group_by(country, District, Locus) %>%  # we group by the District and Locus 
  complete(year = seq(min(min_year_r539t)+6, 2023, 1)) %>% # this creates all the year values up to 2023
  #mutate(adj_year = year - min_year_c580y) %>%
...
all_kelch_pred <- rstanarm::stan_glmer(
  lrsmed_all_kelch ~ adj_year + (1 + adj_year | District),
  data = pfk7_data %>% filter(nobs_kelch > numofobs, year >= min_year_all_kelch & year <= min_year_all_kelch+6),
  weights = wt_med_580y, adapt_delta = 0.9975
)
...
new_data <- pfk7_data %>%
  ...
  group_by(country, District, Locus) %>% # we group by the District and Locus 
  complete(year = seq(min(min_year_all_kelch)+6, 2023, 1))%>% # this creates all the year values up to 2023
  #mutate(adj_year = year - min_year_c580y) %>%
...
#Plotting
```{r}
ex <- pfk7_data %>% filter(is.finite(lrsmed_580y), nobs_c580y > numofobs, year >= min_year_c580y)
df_pred <- df_predict %>% filter(District %in% unique(ex$District)) 
## PLOT FOR C580Y
prev_c580y_plot <- 
  ggplot() +
  geom_line(data=ex %>% mutate(N = n+c580y), aes(year, predict_stan_c580y, color = "true")) + 
  geom_line(data=df_pred %>% filter(Locus == "C580Y"), aes(year,predict , color = "predicted")) +
  geom_vline(data=ex, aes(xintercept= min_year_c580y+6), linetype = "dashed") +
  ggtitle("C580Y") +
  labs(shape="Mutations", size = "Sample Size") +
  theme_bw() +
  #geom_ribbon(aes(ymin = conf.low_c580y, ymax = conf.high_c580y, fill = country, color = country, group=country), alpha = 0.2,  show.legend = FALSE) +
  facet_wrap(~country + District) +
  #scale_y_log10() +
  #scale_color_manual(values=c("#009E73", "#0072B2", "#E69F00", "#56B4E9", "#F0E442")) + 
  #scale_fill_manual(values=c("#009E73", "#0072B2", "#E69F00", "#56B4E9", "#F0E442")) + 
  ylab("Prevalence") +
  xlab("Year") +
...
prev_r539t_plot <- 
 ggplot() +
  geom_line(data=ex %>% mutate(N = n+r539t), aes(year, predict_stan_r539t, color = "true"), size = 1) + 
  geom_line(data=df_pred %>% filter(Locus == "R539T"), aes(year,predict , color = "predicted")) +
  ggtitle("R539T") +
  labs(shape="Mutations", size = "Sample Size") +
  theme_bw() +
  #geom_ribbon(aes(ymin = conf.low_c580y, ymax = conf.high_c580y, fill = country, color = country, group=country), alpha = 0.2,  show.legend = FALSE) +
...
prev_all_kelch_plot <- 
  ggplot() +
  geom_line(data=ex %>% mutate(N = n+all_kelch), aes(year, predict_stan_all_kelch, color = "true"), size = 1) + 
  geom_line(data=df_pred %>% filter(Locus == "all_kelch"), aes(year,predict , color = "predicted")) +
  ggtitle("All kelch13 mutations") +
  labs(shape="Mutations", size = "Sample Size") +
  theme_bw() +
  #geom_ribbon(aes(ymin = conf.low_c580y, ymax = conf.high_c580y, fill = country, color = country, group=country), alpha = 0.2,  show.legend = FALSE) +
  facet_wrap(~country + District) +
...
```{r}
ex <- pfk7_data %>% filter(is.finite(lrsmed_580y), nobs_c580y > numofobs, year >= min_year_c580y) %>% mutate(pt_color = case_when (year>min_year_c580y+6 ~ "#55C667FF", year <= min_year_c580y+6 ~ "#DCE319FF"))
df_pred <- df_predict %>% filter(District %in% unique(ex$District)) 
## PLOT FOR C580Y
prev_c580y_plot <- 
  ggplot() +
  geom_line(data=ex %>% mutate(N = n+c580y), aes(year, predict_stan_prev_c580y, color = "true"), size = 1) + 
  geom_line(data=df_pred %>% filter(Locus == "C580Y"), aes(year, exp(predict)*100/(1+exp(predict)) , color = "predicted"), size=1) +
  geom_vline(data=ex, aes(xintercept= min_year_c580y+6), linetype = "dashed") +
...
ex <- pfk7_data %>% filter(is.finite(lrsmed_539t), nobs_r539t > numofobs, year >= min_year_r539t) %>% mutate(pt_color = case_when (year>min_year_r539t+6 ~ "#55C667FF", year <= min_year_r539t+6 ~ "#DCE319FF"))
df_pred <- df_predict %>% filter(District %in% unique(ex$District))  
prev_r539t_plot <- 
  ggplot() +
  geom_line(data=ex %>% mutate(N = n+r539t), aes(year, predict_stan_prev_r539t, color = "true"), size = 1) + 
  geom_line(data=df_pred %>% filter(Locus == "R539T"), aes(year, exp(predict)*100/(1+exp(predict)) , color = "predicted"), size=1) +
  geom_vline(data=ex, aes(xintercept= min_year_r539t+6), linetype = "dashed") +
...
ex <- pfk7_data %>% filter(is.finite(lrsmed_all_kelch), nobs_kelch > numofobs, year >= min_year_all_kelch) %>% mutate(pt_color = case_when (year>min_year_all_kelch+6 ~ "#55C667FF", year <= min_year_all_kelch+6 ~ "#DCE319FF"))
df_pred <- df_predict %>% filter(District %in% unique(ex$District))  
prev_r539t_plot <- 
  ggplot() +
  geom_line(data=ex %>% mutate(N = n+all_kelch), aes(year, predict_stan_prev_all_kelch, color = "true"), size = 1) + 
  geom_line(data=df_pred %>% filter(Locus == "all_kelch"), aes(year, exp(predict)*100/(1+exp(predict)) , color = "predicted"), size=1) +
  geom_vline(data=ex, aes(xintercept= min_year_all_kelch+6), linetype = "dashed") +
  geom_point(data = ex, aes(x=year, y=prev_539t*100), color = ex$pt_color) +
  ggtitle("R539T") +
  labs(shape="Mutations", size = "Sample Size") +
  theme_bw() +
