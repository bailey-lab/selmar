...
# Forecasting Selection based on the first 2, 3, and 4 years with prevalence cut off
```{r}
pred_year <- c(2, 3, 4)
prev_cutoff <- 0.2
...
for (i in pred_year) {
  #------ Model prediction for C580Y ------
  data_c580y <- pfk7_data %>% filter(
      nobs_c580y > numofobs,
      year >= min_year_c580y & year <= min_year_c580y + i,
      prev_580y < prev_cutoff)
  
  val <- data_c580y[data_c580y$year == data_c580y$min_year,] 
