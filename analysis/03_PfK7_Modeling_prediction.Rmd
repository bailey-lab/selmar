...
new_data <- pfk7_data %>%
  ...
  mutate(adj_year = year - 1993) %>%
  select(adj_year,year, country, District, Locus) # select just the columns needed for the model

predictions <- rstanarm::posterior_predict(c580y_pred, type = "response", newdata = new_data) %>% colMeans
new_data$predict <- predictions
new_data$year <- new_data$adj_year +1993
...
# forecasting
new_data <- pfk7_data %>%
  filter(nobs_r539t > numofobs, is.finite(pfk7_data$lrsmed_539t)) %>% # this is for c469Y model 
  mutate(Locus = "R539T") %>%
  group_by(country, District, Locus) %>% # we group by the District and Locus 
...
prev_c580y_plot <- 
  ggplot() +
  geom_line(data=ex %>% mutate(N = n+c580y), aes(year, predict_stan_c580y, color = "true"), size = 1) + 
  geom_line(data=df_pred %>% filter(Locus == "C580Y"), aes(year,predict , color = "predicted")) +
  geom_vline(data=ex, aes(xintercept= min_year_c580y), linetype = "dashed") +
  ggtitle("C580Y") +
